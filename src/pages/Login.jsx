...
const FEATURES = [
  { icon: "🛡️", title: "Verified Professionals",  desc: "Every cleaner is background-checked & trained" },
  { icon: "⚡", title: "Same-Day Availability",    desc: "Book and get service as early as today"       },
  { icon: "🔄", title: "Free Re-clean Guarantee",  desc: "Not satisfied? We'll redo it at no cost"      },
...
export default function Login() {
  ...
  return (
    /*
      KEY FIX: No p-4 or centering on mobile — the page flows naturally.
      On desktop (lg+) we switch to a side-by-side flex layout.
    */
    <div className="min-h-screen bg-[#f2f6f4] flex flex-col lg:flex-row">

      {/* ════════════════════════════════════════
          LEFT PANEL — desktop only (lg+)
          Full height green branding column
      ════════════════════════════════════════ */}
          ...
          <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center shadow-lg overflow-hidden flex-shrink-0">
            <img
              ...
              onError={(e) => {
                ...
                e.target.parentElement.innerHTML = '<span style="font-size:22px">🧹</span>';
              }}
            />
          </div>
          <div>
            <span className="font-sekuya text-white text-2xl leading-tight block">TidyNest</span>
            <span className="text-emerald-200 text-xs font-poppins">Home Services</span>
          </div>
        </div>

        {/* Hero copy */}
        <div className="z-10 animate-fadeInUp" style={{ animationDelay: "80ms" }}>
          <h1 className="font-sekuya text-white text-4xl xl:text-5xl leading-tight mb-5">
            Clean Homes,<br />Happy Lives
          </h1>
            ...
            {FEATURES.map(({ icon, title, desc }, i) => (
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-emerald-200 text-xs font-poppins mt-0.5">{desc}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        {/* ── Mobile-only green header ──
            Sits flush at the very top, no outer padding.
            Hidden on lg+ (desktop shows the left panel instead).
        */}
        <div className="lg:hidden bg-gradient-to-br from-[#0a7a53] to-[#065e3f] text-center px-6 pt-12 pb-12 rounded-b-[2.5rem] shadow-lg">
          ...
          <h1 className="text-white text-3xl font-sekuya">TidyNest</h1>
          <p className="text-emerald-200 text-sm mt-2 font-poppins">Your trusted home cleaning partner</p>

          {/* Mini feature pills on mobile */}
          <div className="flex justify-center gap-2 mt-5 flex-wrap">
            {["🛡️ Verified", "⚡ Same Day", "🔄 Free Re-clean"].map((f) => (
              <span
                key={f}
        ...
        {/* ── Form area ──
            On mobile: natural top-to-bottom flow with padding.
            On desktop: fills remaining height and centers form vertically.
        */}
        <div className="flex-1 flex flex-col lg:justify-center px-5 pt-8 pb-10 lg:px-14 xl:px-20 bg-white lg:bg-[#f2f6f4]">
          <div className="w-full max-w-md mx-auto animate-fadeInUp">
            ...
            {role === "user" ? (
              <div className="space-y-5 animate-fadeInUp">
                ...
                <div>
                  ...
                  />
                  {errors.name && (
                    <p className="text-red-500 text-xs mt-1.5 font-poppins">⚠️ {errors.name}</p>
                  )}
                </div>
                ...
                <div>
                  ...
                  {errors.phone && (
                    <p className="text-red-500 text-xs mt-1.5 font-poppins">⚠️ {errors.phone}</p>
                  )}
                </div>
              ...
              <div className="space-y-5 animate-fadeInUp">
                <div className="bg-amber-50 border border-amber-100 rounded-xl px-4 py-4 flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-amber-700 font-montserrat">Admin Access Only</p>
                ...
                <div>
                  ...
                  {errors.password && (
                    <p className="text-red-500 text-xs mt-1.5 font-poppins">⚠️ {errors.password}</p>
                  )}
                  <p className="text-xs text-gray-400 mt-1.5 font-poppins">Hint: admin123</p>
                </div>
            ...
            <p className="text-center text-xs text-gray-400 mt-8 font-poppins">
              ...
              <span className="text-[#0a7a53] font-medium cursor-pointer hover:underline">Terms</span>
              {" & "}
              <span className="text-[#0a7a53] font-medium cursor-pointer hover:underline">Privacy Policy</span>
            </p>
