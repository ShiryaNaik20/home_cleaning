...
export default function ServiceListing() {
  ...
  return (
    <div className="bg-[#f2f6f4] min-h-screen pb-28">
      ...
      {/* ── Hero Header: only text + search inside green ── */}
      ...
      <div className="max-w-4xl mx-auto px-4 mt-7 space-y-5">
        ...
        <div className="grid grid-cols-3 gap-3 stagger">
          {[
            ...
            >
              ...
              <p className="text-xs font-bold text-gray-800 font-montserrat">{label}</p>
              <p className="text-[10px] text-gray-400 font-medium font-poppins">{sub}</p>
            </div>
          ))}
        </div>

        {/* Promo Banner */}
        <div className="bg-[#0a7a53] text-white rounded-3xl p-5 relative overflow-hidden flex items-center justify-between shadow-sm animate-fadeInUp">
          <div className="space-y-1.5 z-10 max-w-[65%]">
            <span className="bg-emerald-800/80 text-emerald-200 text-[10px] uppercase font-bold px-2.5 py-0.5 rounded-full font-poppins">
              Limited Offer
            </span>
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none animate-fadeInUp">
          <button
            onClick={() =>
              setSortBy((current) =>
                current === "rating" ? "price_low" : current === "price_low" ? "price_high" : "rating"
              )
            }
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-3xl border border-emerald-900/5 animate-fadeIn">
            ...
            <p className="text-gray-700 font-bold font-montserrat">No services found</p>
            <p className="text-gray-400 text-xs mt-1 font-poppins">Try updating your search or filter</p>
          </div>
        ) : (
